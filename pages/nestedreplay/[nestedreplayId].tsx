...
const NestedreplayId = () => {
  const router = useRouter();
  const { nestedreplayId } = router.query;
  const { data: nestedReplay, mutate: nestedMutatedReplay } = useNestedReplay(
    nestedreplayId as string
  );
      ...
      <div className=" w-full p-2 my-1 cursor-pointer mt-2">
        ...
        <div className="flex flex-col w-full">
          <div className="w-full flex gap-2  mt-5">
            ...
            <textarea
              className="w-full h-20 border-2 border-solid border-gray-300 disabled:opacity-80 resize-none  
              mt-3 p-2 rounded-lg text-[20px] placeholder-gray-400 focus:outline-none ring-0 outline-none"
              placeholder="Replay"
            />
          </div>
          <div className="flex justify-end mt-1">
            <button className="bg-sky-500 rounded-full h-11 p-2 text-center text-white text-lg font-semibold w-20">
              Replay
            </button>
          </div>
        </div>
