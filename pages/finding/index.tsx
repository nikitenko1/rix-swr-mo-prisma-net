...
const Finding = () => {
  ...
  return (
    <>
      <Head>
        <title>Finding | UA-You</title>
      </Head>
      {searchResult?.posts.length === 0 && searchResult?.users.length === 0 && (
        <h1 className="text-center mt-5 text-xl">No Result Found</h1>
      )}
      {searchResult?.users.length > 0 &&
        searchResult?.users?.map((user: User) => (
          <div
            key={user.id}
            className="flex flex-row gap-4 ml-5 mt-5 justify-start"
              ...
              <div className=" mx-5  p-3">
                <div>
                  {!linkRegex.test(post.body) && !mentionRegex.test(post.body) && (
                    <p className="text-md text-black break-words">{post.body}</p>
                  )}
                  {post.body.match(linkRegex) && !mentionRegex.test(post.body) && (
                    <div className="mt-2">
                      ...
                      {Array.from(post.body.matchAll(linkRegex)).map((link: any, index) => (
                        <li className="list-none" key={index}>
                          <span className="text-blue-500 hover:underline">{link[0]}</span>
                        </li>
                      ))}
                    </div>
                  )}
