  ...
  if (req.method !== "POST" && req.method !== "GET" && req.method != "DELETE")
    ...
    try {
      ...
      if (req.method === "POST") {
        ...
        try {
          const mentionUser = body.match(mentionRegex);
          if (mentionUser) {
            const user = await prisma.user.findUnique({
              where: {
                customTag: mentionUser[0],
              },
            });
        ...
        else if (replay.userId != currentUser.id)
          throw new Error("You are not authorized to delete this replay");
        else {
