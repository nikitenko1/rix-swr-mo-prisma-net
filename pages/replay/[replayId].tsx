...
const ReplayId = () => {
  ...
  return (
    <div className="flex flex-col items-start p-2 w-full  my-2 mx-2 " key={replay?.id}>
      ...
      <div className=" mx-10">
        <>
          {replay && !linkRegex.test(replay.body) && !mentionRegex.test(replay.body) && (
            <p className="text-md text-black break-words">{replay.body}</p>
          )}
            ...
            <div>
              ...
              {Array.from(replay.body.matchAll(linkRegex)).map((link, index) => (
                <li className="list-none" key={index}>
                  <Link href={link[0].includes("http") ? link[0] : `https://${link[0]}`}>
