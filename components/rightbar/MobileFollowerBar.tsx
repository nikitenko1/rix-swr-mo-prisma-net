      ...
      {currentUser && (
        <div className="sm:flex md:hidden mt-2 ml-5">
          <div className="flex flex-col">
            {url === "/" && (
              <h1 className="font-bold text-2xl text-center">Who To Follow Mobile ...</h1>
            )}
            {url === "/" &&
              users &&
              users.length < 10 &&
              users?.map((user: User) => (
                <div key={user?.id} className="flex-col justify-center items-center w-auto">
                  ...
                  {currentUser.user.id !== user.id && (
                    <div className="mx-2">
                      <button
                        className="text-blue w-full border-2 border-solid  p-1  my-3 mr-3 border-blue-400  text-lg font-medium rounded-full"
            ...
            {users && users.length > 10 && url === "/" && (
              <>
                ...
                <h1
                  className="text-lg text-black cursor-pointer hover:underline text-center"
                  onClick={() => router.push("/usersuggestions")}
                >
                  Show more -- UserSuggestions
                </h1>
              </>
            )}
          </div>
