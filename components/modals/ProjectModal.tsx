...
const ProjectModal = () => {
  ...
  years.sort((a, b) => b - a);
  const handleStartMonth = (event: ChangeEvent<HTMLOptionElement>) => {
    setStartMonth(event.target.value);
  };

  const handleStartYear = (event: ChangeEvent<HTMLOptionElement>) => {
    setStartYear(event.target.value);
  };

  const handleEndMonth = (event: ChangeEvent<HTMLOptionElement>) => {
    setEndMonth(event.target.value);
  };

  const handleEndYear = (event: ChangeEvent<HTMLOptionElement>) => {
    ...
    <div className="w-full gap-5">
      <div className="flex flex-col gap-2">
        ...
        <input
          ...
          className="border-2 p-2border-2 border-gray-200 rounded-lg w-full p-2 
        ...
        <div className="flex gap-2 mt-2">
          <select className="w-2/4 h-10">
            ...
            {months.map((month, index) => (
              <option key={index} value={month} onChange={handleStartMonth}>
                {month}
              </option>
            ))}
          </select>
          <select className="w-2/4 h-10">
            ...
            {years.sort() &&
              years.map((year, index) => (
                <option key={index} value={year} onChange={handleStartYear}>
                  {year}
                </option>
              ))}
          </select>
      ...
      <div className="mt-5">
        ...
        <div className="flex gap-2 mt-2">
          <select className="w-2/4 h-10">
            <option>Month</option>
            {months.map((month, index) => (
              <option key={index} value={month} onChange={handleEndMonth}>
                {month}
              </option>
            ))}
          </select>
          <select className="w-2/4 h-10">
            ...
            {years.sort() &&
              years.map((year, index) => (
                <option key={index} value={year} onChange={handleEndYear}>
                  {year}
                </option>
              ))}
          </select>
