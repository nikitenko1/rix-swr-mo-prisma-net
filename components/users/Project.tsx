      ...
      {projects && projects.length > 2 ? (
        <>
          {projects.slice(0, 2).map((project) => (
            <div
              className="flex justify-between items-center gap-2 w-auto my-2 mx-1"
              key={project.id}
            >
              <div className="flex flex-col items-center p-2 cursor-pointer w-auto">
                ...
                <Link
                  className="flex gap-2 border-2 rounded-full items-center border-gray-400 w-auto p-2 mt-2
                  hover:border-black"
                  href={project.link}
                >
          ...
          <h1
            ...
            onClick={() => router.push(`/user/${userId}/projects`)}
          >
            Show all {projects.length} more
          </h1>
        </>
      ) : (
        projects &&
