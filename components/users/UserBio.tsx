          ...
          <>
            <div className="flex items-center gap-2">
              {isFollower && (
                <BiMessageSquareDots
                  size={30}
                  className="text-gray-500 border-2 border-solid rounded-md border-gray-500 cursor-pointer"
                  title="message"
              ...
              <button
                className={` text-black font-bold py-2 px-4 rounded-full ${
                  isFollowing ? "hover:text-red-500" : ""
                }
                   ${
                     isFollowing
                       ? "border-2 border-solid border-red-500"
                       : "border-2 border-solid rounded-lg border-blue-300 hover:border-gray-200 p-2"
                   }
                   `}
      ...
      <div className="mt-3 px-4">
        <div className="flex flex-col">
          <div className="flex items-center">
            ...
            {fetchUser?.isVerified && <MdVerified className="text-blue-500  md:ml-2" />}
          </div>

          <p className="text-gray-500">{fetchUser?.customTag}</p>
          {fetchUser?.bio && !linkRegex.test(fetchUser.bio) && (
            ...
            <div className="mt-0">
              ...
              {Array.from(fetchUser?.bio.matchAll(linkRegex)).map((link, index) => (
                <li className="list-none" key={index}>
                  <a href={link[0]} target="_blank" className="text-blue-500 hover:underline">
                    {link[0].split("https://")[1]}
                  </a>
                </li>
              ))}
            </div>
          ...
          {fetchUser && (
            <p>
              ...
              <span
                className="font-bold  ml-3 cursor-pointer hover:underline"
                onClick={() => router.push(`/user/${userId}/followers`)}
              >
                {fetchUser?.followerId.length}
              </span>{" "}
              <span
                className="text-gray-500 cursor-pointer hover:underline"
                onClick={() => router.push(`/user/${userId}/followers`)}
