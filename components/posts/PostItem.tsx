...
const PostItem = ({ post, mutate, userId }: IProps) => {
  ...
  const fromPostCreatedAt = useMemo(() => {
    if (!post?.postSharedCreatedAt) return "";
      ...
      // the addSuffix option to true, which will add the words "ago" or "in" to the output
      // depending on whether the date is in the past or future
      return formatDistanceToNowStrict(new Date(post.postSharedCreatedAt), {
        addSuffix: true,
      });
    }
  ...
  return (
    <div
      onClick={gotoPost}
      className="flex flex-col border-2 border-solid border-gray-300 p-5 cursor-pointer rounded-lg
      my-5 mx-2 hover:border-gray-200"
      key={post.id}
    >
      <div className="flex items-start gap-1">
        <Avatar userId={post?.userId} />
        <div className="flex items-center md:gap-2 ">
          <p className="hidden md:block text-md  font-bold hover:underline" onClick={gotoProfile}>
            {post.user.name}
          </p>
          <p
          ...
          {currentUser?.user?.id === post?.userId && (
            <AiOutlineEdit size={20} className="text-gray-500 " title="Edit" onClick={onOpen} />
          )}
          {currentUser?.user?.id === post?.userId && (
            <AiOutlineDelete
              size={20}
      ...
      <div className="mx-5 p-1">
        <>
          {!linkRegex.test(post.body) && !mentionRegex.test(post.body) && (
            <p className={`text-lg  p-2 font-bold text-gray-500  ${post.isShared && "mx-5 mt-0"}`}>
              {post.body}
            </p>
          )}
            ...
            <div className=" flex flex-col">
              ...
              {Array.from(post.body.matchAll(linkRegex)).map((link, index) => (
                <li className="list-none" key={index}>
                  <span className="text-blue-500 break-words hover:underline">{link[0]}</span>
                </li>
              ))}
            </div>
          )}
          ...
          {post.body.match(mentionRegex) && post.body.match(linkRegex) && (
            <>
              <div className="flex items-center gap-1 w-3/4">
                ...
                <div className="text-lg font-bold text-gray-500 mr-2  border-2 whitespace-nowrap">
      ...
      {post.image && (
        <CldImage
          src={post.image}
          alt="post"
          className={` object-cover mb-3 ${post.isShared && "mx-5 mt-0"}`}
          width="500"
          height="500"
        />
      )}
